...
def simulate_abp(N, dt, v, W, D_T, D_R, x0=0.0, y0=0.0, theta0=0.0, seed=None):
    ...
    W       : angular velocity (chirality) [rad/s]
    D_T     : translational diffusion coefficient [m²/s]
    D_R     : rotational diffusion coefficient [rad²/s]
    x0,y0   : initial position [m]
    theta0  : initial orientation [rad]
...
def ensemble_msd(N_steps, N_particles, dt, v, W, D_T, D_R, seed=0):
    """Vectorised ensemble mean-square displacement.

    Parameters
    ----------
    N_steps     : number of time steps
    N_particles : ensemble size
    (remaining parameters same as simulate_abp)

    Returns
    -------
    tau : time-lag array [s]
    msd : ensemble-averaged MSD [m²]
    """
    rng = np.random.default_rng(seed)
    xe = np.zeros(N_particles)
    ye = np.zeros(N_particles)
    te = rng.uniform(0, 2 * np.pi, N_particles)   # random initial angles
    s2T = np.sqrt(2 * D_T * dt)
    s2R = np.sqrt(2 * D_R * dt)
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 1 — Single-particle trajectory
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 2 — Velocity autocorrelation function
# ─────────────────────────────────────────────────────────────────────────────
...
def plot_vacf(x, y, N, dt, D_R, v, max_lag=2000, save='vacf.png'):
    ...
    plt.plot(tau, Cv_theory, color='tomato',    lw=1.5, ls='--',
...
def plot_msd(N_particles, N_steps, dt, v, W, D_T, D_R, seed=0, save='msd.png'):
    ...
    msd_th       = msd_theory(tau, D_T, D_R, v)
    msd_ballistic = v**2 * tau**2
    D_eff         = D_T + v**2 / (2 * D_R)
    msd_diffusive = 4 * D_eff * tau

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.loglog(tau[1:], msd[1:]*1e12,          color='steelblue', lw=1.5, label='simulation')
    ax.loglog(tau[1:], msd_th[1:]*1e12,       color='tomato',    lw=2,   ls='--', label='theory')
    ax.loglog(tau[1:], msd_ballistic[1:]*1e12, color='green',     lw=1,   ls=':',
              label=r'ballistic $\propto\tau^2$')
    ax.loglog(tau[1:], msd_diffusive[1:]*1e12, color='orange',    lw=1,   ls=':',
              label=r'diffusive $\propto\tau$')
